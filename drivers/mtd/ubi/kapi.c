...
/**
 * ubi_leb_secure_write - write data securely.
 * @desc: volume descriptor
 * @lnum: logical eraseblock number to write to
 * @buf: data to write
 * @offset: offset within the logical eraseblock where to write
 ...
 * returns immediately with %-EBADF code.
 */
int ubi_leb_secure_write(struct ubi_volume_desc *desc, int lnum,
			 const void *buf, int offset, int len)
{
	...
	struct mtd_info *mtd = ubi->mtd;
	int vol_id = vol->vol_id;
	int ngroups = mtd_pairing_groups_per_eb(mtd);
	int usable_leb_size = (ubi->peb_size / ngroups) -
			      ubi->leb_start - vol->data_pad;
	struct nand_pairing_info info;
	int err, pair, wunit;
	...
	offset += ubi->leb_start;
	info.group = 0;
	info.pair = offset / ubi->min_io_size;

	while (len) {
		wunit = mtd_pairing_info_to_wunit(mtd, &info);
		offset = (wunit * ubi->min_io_size) - ubi->leb_start;

		err = ubi_eba_write_leb(ubi, vol, lnum, buf, offset,
					ubi->min_io_size);
		if (err)
			return err;
...
EXPORT_SYMBOL_GPL(ubi_leb_secure_write);

/**
 * ubi_leb_secure_read - read data securely.
 * @desc: volume descriptor
 * @lnum: logical eraseblock number to read from
 * @buf: buffer where to store the read data
 * @offset: offset within the logical eraseblock to read from
 ...
 * returns immediately with %-EBADF error code.
 */
int ubi_leb_secure_read(struct ubi_volume_desc *desc, int lnum, char *buf,
			int offset, int len, int check)
{
	struct ubi_volume *vol = desc->vol;
	struct ubi_device *ubi = vol->ubi;
	...
	while (len) {
		...
		err = ubi_eba_read_leb(ubi, vol, lnum, buf, offset,
				       ubi->min_io_size, check);
		if (err) {
...
EXPORT_SYMBOL_GPL(ubi_leb_secure_read);

static int ubi_ptr_check(struct ubi_ptr *ptr, int len)
{
	struct ubi_volume_desc *desc = ptr->desc;
	struct ubi_volume *vol = desc->vol;
	struct ubi_device *ubi = vol->ubi;
	struct mtd_info *mtd = ubi->mtd;

	if (!ptr || ptr->desc || ptr->lnum < 0)
		return -EINVAL;

	if (ptr->offset + len > vol->usable_leb_size)
		return -EINVAL;

	if ((ptr->options & UBI_PTR_SECURE_MODE) &&
	    mtd_pairing_groups_per_eb(mtd) > 1) {
		int wunit = (ptr->offset + ubi->leb_start) / ubi->min_io_size;
		struct nand_pairing_info info;
		int npairs = mtd_pairing_pairs_per_eb(mtd);
		int nwunits;

		mtd_wunit_to_pairing_info(mtd, wunit, &info);

		/*
		 * The user asked for secure access mode and the offset points
		 * to a page in group > 0 (i.e. write access to this page can
		 * corrupt the page attached to group 0 in the same pair).
		 */
		if (info.group)
			return -EINVAL;


		len -= ubi->min_io_size - (ptr->offset % ubi->min_io_size);
		nwunits = DIV_ROUND_UP(len, ubi->min_io_size) + 1;
		if (nwunits + info.pair >= npairs)
			return -EINVAL;
	}
...
int ubi_ptr_find_contiguous(struct ubi_ptr *ptr, int len)
{
	...
	struct mtd_info *mtd = ubi->mtd;
	struct nand_pairing_info info;
	int wunit, nwunits, avail, offset;
	...
	if ((ptr->offset + len) / ubi->min_io_size ==
	    ptr->offset / ubi->min_io_size)
		return ptr->offset;

	if (!(ptr->options & UBI_PTR_SECURE_MODE) ||
	    mtd_pairing_groups_per_eb(mtd))
		return ptr->offset;

	offset = ptr->offset;
	avail = ubi->min_io_size - (ptr->offset % ubi->min_io_size);
	nwunits = (mtd->erasesize - vol->data_pad) / mtd->writesize;
	wunit = (ptr->offset + avail) / ubi->min_io_size;
	for (; wunit < nwunits; wunit++)
	{
		mtd_wunit_to_pairing_info(mtd, wunit, &info);
		if (info.group) {
			avail = 0;
...
int ubi_ptr_contiguous_len(struct ubi_ptr *ptr)
{
	...
	if (ptr->offset > vol->usable_leb_size)
		return -EINVAL;

	if (!(ptr->options & UBI_PTR_SECURE_MODE) ||
	    mtd_pairing_groups_per_eb(mtd))
		return vol->usable_leb_size - ptr->offset;

	nwunits = (mtd->erasesize - vol->data_pad) / mtd->writesize;
	wunit = (ptr->offset + ubi->leb_start + ubi->min_io_size - 1) /
		ubi->min_io_size;
	for (; wunit < nwunits; wunit++)
	{
		mtd_wunit_to_pairing_info(mtd, wunit, &info);
	...
	return (wunit * ubi->min_io_size) - ptr->offset - ubi->leb_start;
}
EXPORT_SYMBOL_GPL(ubi_ptr_contiguous_len);
...
int ubi_ptr_skip_len(struct ubi_ptr *ptr)
{
	...
	struct mtd_info *mtd = ubi->mtd;
	struct nand_pairing_info info;
	int wunit, nwunits, skipped_wunits = 0;
	...
	return (skipped_wunits * ubi->min_io_size);
}
EXPORT_SYMBOL_GPL(ubi_ptr_skip_len);
...
int ubi_ptr_adjust_offset(struct ubi_ptr *ptr, int offset)
{
	...
	if (offset % ubi->min_io_size)
		return -EINVAL;

	nwunits = mtd->erasesize / mtd->writesize;
	for (wunit = (offset + ubi->leb_start) / ubi->min_io_size;
	     wunit < nwunits; wunit++)
	{
		mtd_wunit_to_pairing_info(mtd, wunit, &info);
		if (!info.group)
			return (wunit * ubi->min_io_size) - ubi->leb_start;
	}

	return -EINVAL;
}
EXPORT_SYMBOL_GPL(ubi_ptr_adjust_offset);
...
int ubi_ptr_read(struct ubi_ptr *ptr, void *buf, int len)
{
	...
	struct mtd_info *mtd = ubi->mtd;
	struct nand_pairing_info info;
	int err, wunit, ret = 0, vol_id = vol->vol_id;

	err = ubi_ptr_check(ptr, len);
	if (err)
		return err;

	if (!(ptr->options & UBI_PTR_SECURE_MODE)) {
		err = ubi_leb_read(ptr->desc, ptr->lnum, buf, ptr->offset,
				   len, ptr->options & UBI_PTR_CHECK);
		if (err)
			return err;
	...
	wunit = (ptr->offset + ubi->leb_start) / ubi->min_io_size;
	mtd_wunit_to_pairing_info(mtd, wunit, &info);

	while (len) {
		int chunk_len = ubi->min_io_size -
				(ptr->offset % ubi->min_io_size);

		if (chunk_len < len)
			chunk_len = len;

		err = ubi_eba_read_leb(ubi, vol, ptr->lnum, buf, ptr->offset,
				       chunk_len,
				       ptr->options & UBI_PTR_CHECK);
		if (err) {
			if (!mtd_is_eccerr(err))
				return err;
		...
		len -= chunk_len;
		ptr->offset += chunk_len;

		if (!(ptr->offset % ubi->min_io_size)) {
			info.pair++;
			wunit = mtd_pairing_info_to_wunit(mtd, &info);
			ptr->offset = (wunit * ubi->min_io_size) -
				      ubi->leb_start;
		}
	}
...
int ubi_ptr_write(struct ubi_ptr *ptr, const void *buf, int len)
{
	...
	struct mtd_info *mtd = ubi->mtd;
	struct nand_pairing_info info;
	int err, wunit, vol_id = vol->vol_id;
	...
	if (!(ptr->options & UBI_PTR_SECURE_MODE)) {
		err = ubi_leb_write(ptr->desc, ptr->lnum, buf,
				    ptr->offset, len);
		if (err)
			return err;
	...
	while (len) {
		...
		err = ubi_eba_write_leb(ubi, vol, ptr->lnum, buf, ptr->offset,
				       chunk_len);
		if (err)
			return err;
